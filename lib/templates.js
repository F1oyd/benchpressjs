...
(function(module) {
	...
	function replace(string, regex, value) {
		return string.replace(regex, function() {
			return value.toString().replace(/\$/g, '<!-- dollar -->');
		});
    }
    ...
    var replaceChar = function(c) { return escapeCharMap[c] };
    var escapeChars = /[&<>"'`=]/g;
	...
	function replaceValue(template, key, value) {
        ...
        string = replace(string, new RegExp('{-' + key + '-}', 'g'), value
            .replace(escapeChars, replaceChar)
        );
		...
		for (var iterator in array) {
			if (array.hasOwnProperty(iterator)) {
				...
				if (!isObject) {
					...
					result = checkConditionals(result, '@value', !!array[iterator]);
	...
	function cleanup(template, obj) {
		...
		if (parseUndefined) {
			return parse(template, undefinedKeys, '');
		} else {
			return template.replace(/<!-- dollar -->/g, '$');
		}
	}
